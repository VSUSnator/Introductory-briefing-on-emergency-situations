...
// Устанавливаем часовой пояс на Иркутск
    ...
    if (isset($_SESSION['user'])) {
        ...
        foreach ($existingData as $index => $row) {
            if ($row[1] === $phoneNumber) {
                ...
                // Записываем время, когда материал был пройден
                $row[$materialIndex + 9] = date('Y-m-d H:i:s'); // Сохраняем дату и время в формате 'ГГГГ-ММ-ДД ЧЧ:ММ:СС'
        ...
        if ($userUpdated) {
            ...
            if (count($existingData) === 0 || !isset($existingData[0][0]) || empty($existingData[0][0])) {
                $header = ['ФИО', 'Номер телефона', 'Профессия', 'Дата регистрации', 
                           ...
                           'Материал 4', 'Материал 5', 'Дата прохождения']; // Добавлена новая колонка для даты прохождения
