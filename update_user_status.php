...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_SESSION['user'])) {
        ...
        $materialIndex = $_POST['material_index']; // Индекс материала (0, 1 или 2)
        ...
        foreach ($existingData as $index => $row) {
            if ($row[1] === $phoneNumber) {
                ...
                $row[$materialIndex + 4] = 'пройден'; // +3, чтобы учесть первые три колонки
        ...
        if ($userUpdated) {
            ...
            if (count($existingData) === 0 || !isset($existingData[0][0]) || empty($existingData[0][0])) {
                $header = ['ФИО', 'Номер телефона', 'Профессия', 'Дата регистрации', 'Гражданская оборона и ЧС', 'Антитеррористическая защищенность', 'Инструкция о порядке действия сотрудников при пожаре', 'Материал 1', 'Материал 2', 'Материал 3'];
